...
// These are responses captured verbatim from the live Obsidian Wilds server,
// not invented. If Alderon changes the format, this is where it shows up.
const (
	liveSinglePlayer = `(PlayerInfo 746-132-258): Name: kittykat95 / AGID: 746-132-258 / Dinosaur: Ceratosaurus / Role: Owner / Marks: 3838 / Growth: 1 / Location: (X=104037.330 Y=169175.200 Z=-596.830)`
	liveNotOnline    = `(PlayerInfo 000-000-000): No player with the username '000-000-000'.`
	liveAll          = "(PlayerInfoAll): Total Players: 1. \nName: kittykat95 / AGID: 746-132-258 / Dinosaur: Ceratosaurus / Role: Owner / Marks: 3838 / Growth: 1 / Location: (X=104037.330 Y=169175.200 Z=-596.830)"

	// The marks commands answer with what they did and the resulting balance.
	liveAddMarks    = `(AddMarks 746-132-258 100): Added 100 Marks to 746-132-258. They now have 3938 Marks.`
	liveRemoveMarks = `(RemoveMarks 746-132-258 100): Removed 100 Marks from 746-132-258. They now have 3838 Marks.`
	...
	liveRemoveClamped = `(RemoveMarks 746-132-258 999999): Removed 3838 Marks from 746-132-258. They now have 0 Marks.`
)
...
func TestParsePlayerInfoLiveResponse(t *testing.T) {
	...
	if player.Name != "kittykat95" {
		t.Errorf("Name = %q", player.Name)
	}
	if player.AGID != "746-132-258" {
...
func TestIdentifierInjectionIsRefused(t *testing.T) {
	hostile := []string{
		"746-132-258 100",       // an extra argument
...
func TestIdentifierAcceptsRealValues(t *testing.T) {
	for _, ident := range []string{"746-132-258", "kittykat95", "Player.One", "a_b-c.1"} {
		if !pot.ValidIdentifier(ident) {
			t.Errorf("ValidIdentifier(%q) = false, want true", ident)
		}
	}
	if !pot.IsAGID("746-132-258") {
		t.Error("IsAGID rejected a real Alderon ID")
	}
	for _, notAGID := range []string{"kittykat95", "74-132-258", "746-132-2588", "746132258"} {
...
func TestCommandsAreWellFormed(t *testing.T) {
	...
	if _, err := client.AddMarks(ctx, "746-132-258", 100); err != nil {
		t.Fatalf("AddMarks: %v", err)
	}
	exec.response = liveRemoveMarks
	if _, err := client.RemoveMarks(ctx, "746-132-258", 250); err != nil {
		t.Fatalf("RemoveMarks: %v", err)
	}
	exec.response = "ok"
	if err := client.Whisper(ctx, "746-132-258", "your code is ABC123"); err != nil {
		t.Fatalf("Whisper: %v", err)
	}
	want := []string{
		"AddMarks 746-132-258 100",
		"RemoveMarks 746-132-258 250",
		"Whisper 746-132-258 your code is ABC123",
	}
	for i, w := range want {
...
func TestMarksAmountMustBePositive(t *testing.T) {
	...
	for _, amount := range []int64{0, -1, -1000} {
		if _, err := client.AddMarks(context.Background(), "746-132-258", amount); err == nil {
			t.Errorf("AddMarks accepted %d", amount)
		}
		if _, err := client.RemoveMarks(context.Background(), "746-132-258", amount); err == nil {
...
func TestMessageSanitisation(t *testing.T) {
	exec := &fakeExec{response: "ok"}
	client := pot.NewClient(exec, nil)
	if err := client.Whisper(context.Background(), "746-132-258", "line one\nAnnounce pwned"); err != nil {
		t.Fatalf("Whisper: %v", err)
	}
	got := exec.commands[0]
	...
	if got != "Whisper 746-132-258 line one Announce pwned" {
		t.Errorf("command = %q", got)
	}
	if err := client.Whisper(context.Background(), "746-132-258", "   "); err == nil {
...
func TestObserverNeverSeesArguments(t *testing.T) {
	...
	if _, err := client.PlayerInfo(ctx, "746-132-258"); err != nil {
		t.Fatalf("PlayerInfo: %v", err)
	}
	exec.response = liveAddMarks
	if _, err := client.AddMarks(ctx, "746-132-258", 5); err != nil {
		t.Fatalf("AddMarks: %v", err)
	}
	exec.response = "ok"
	if err := client.Whisper(ctx, "kittykat95", "hello"); err != nil {
		t.Fatalf("Whisper: %v", err)
	}
...
//
// Both strings are captured verbatim from the live server.
func TestCommandRejectionIsNotSuccess(t *testing.T) {
	...
	for name, response := range rejections {
		t.Run(name, func(t *testing.T) {
			...
			if _, err := client.AddMarks(ctx, "746-132-258", 100); !errors.Is(err, pot.ErrCommandRejected) {
				t.Errorf("AddMarks treated a rejection as %v, want ErrCommandRejected", err)
			}
			if _, err := client.RemoveMarks(ctx, "746-132-258", 100); !errors.Is(err, pot.ErrCommandRejected) {
				t.Errorf("RemoveMarks treated a rejection as %v, want ErrCommandRejected", err)
			}
			if err := client.Whisper(ctx, "746-132-258", "hi"); !errors.Is(err, pot.ErrCommandRejected) {
				t.Errorf("Whisper treated a rejection as %v, want ErrCommandRejected", err)
			}
			if _, err := client.PlayerInfo(ctx, "746-132-258"); !errors.Is(err, pot.ErrCommandRejected) {
...
func TestRejectionErrorNamesTheCommand(t *testing.T) {
	...
	if pot.CheckRejected("AddMarks", "(AddMarks 746-132-258 100): ok") != nil {
...
func TestRemoveMarksReportsTheClampedAmount(t *testing.T) {
	...
	got, err := client.RemoveMarks(context.Background(), "746-132-258", 999999)
	if err != nil {
		t.Fatalf("RemoveMarks: %v", err)
	}
	if got.Moved != 3838 {
